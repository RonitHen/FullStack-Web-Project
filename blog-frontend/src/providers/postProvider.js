...
export function PostProvider({children}){
    ...
    const updatePost = async (updatedPost) => {
        try {
            const res = await axios.put(`http://localhost:4000/api/posts/${updatedPost.id}`, updatedPost, {withCredentials: true});
            await fetchPosts();
    ...
    const values = {posts, setPosts, fetchPosts, getPost, addPost, updatePost, removePost};
